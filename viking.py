...
def calc_cloudiness(cloudiness, cloud_dev):
	delta = rd.gauss(0, cloud_dev)
...
def step(equ, cal, cord, tour, h, vel_avg, vel_std, vel_max, cloud_dev, night_navigation,
    ...
    if(t % 60 == 0):
        cloudiness = calc_cloudiness(cloudiness, cloud_dev)
...
def route(equ_or_sol, crystal, days, h,
            vel_avg=11.0, vel_std=2.0, vel_max=21.0, cloud_dev=2,
    ...
    while (t < (24 * 60 * days)) and not(hit):
    	...
    	pos_arr.append(pos)
    	vel_arr.append(vel)
    	t, meas_time, time_until_meas, wake_up_time, change_wake_up_time, elevation, cloudiness, n_error, pos, vel, hit, min_distance = step(equ, cal, cord, tour, h, vel_avg, vel_std, vel_max, cloud_dev, night_navigation, t, meas_time, time_until_meas, wake_up_time, change_wake_up_time, elevation, cloudiness, n_error, pos, vel, hit, min_distance)
    ...
    for l in range(1):
        ...
        for m in range(5):
            ...
            for i in range(simulation_count):
                t_arr, elevation_arr, cloudiness_arr, n_error_arr, pos_arr, vel_arr, hit = route(simulation_equinox, simulation_crystal, 16, simulation_h)

                filename = "simulations/" + simulation_equinox + "_" + simulation_crystal + "_" + str(simulation_h) + "-" + str(i).zfill(5) + ".dat"
                print(simulation_equinox + '\t' + simulation_crystal + '\t' + str(simulation_h) + '\t' + str(i))
            ...
            result_file.write(simulation_equinox + '\t' + simulation_crystal + '\t' + str(simulation_h) + '\t' + str(hit_percent) + '\n')
